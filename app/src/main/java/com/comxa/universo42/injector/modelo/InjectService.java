...
public class InjectService implements Runnable, Loggable {
    ...
    public InjectService(Host hostDest, Host hostCliente) {
        this.hostDest = hostDest;
        this.hostCliente = hostCliente;
    }
    
    public InjectService(Host hostDest, Host hostCliente, int id) {
    ...
    public void run() {
        try {
            try {
                InjectUtil reqCliente = getRequisicao(this.hostCliente);
                reqCliente.setPayload(this.payload);

                onLogReceived("<-> Thread "+this.id+": abrindo comuni��o com proxy.", LOG_LEVEL_INFO, null);
                ...
                if (statusLine.contains(CONNECT_ESTABLISHED)) {
                    ...
                    connect = new ConnectMaker(this.hostDest, this.hostCliente) {
                        ...
                        public void onLogReceived(String log, int level, Exception e) {
                            InjectService.this.onLogReceived(log, level, e);
                        }
                ...
                if (connect == null) {
                    close();
                    onLogReceived("<-> Thread "+this.id+": conex�o encerrada.", LOG_LEVEL_INFO, null);
                    onConnectionClosed();
                }
            }
    ...
    private InjectUtil getRequisicao(Host host) throws IOException {
        InjectUtil reqCliente;

        try {
            reqCliente = new InjectUtil();
    ...
    private int getContentLength(InjectUtil req) {        
