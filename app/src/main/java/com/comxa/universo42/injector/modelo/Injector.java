...
public class Injector implements Runnable,Loggable
{
    ...
    private List<InjectService> connections = new LinkedList<InjectService>();
    ...
    public synchronized void stop() {
        ...
        for (InjectService ri : connections)
			ri.stop();
    }
        ...
        try {
            ...
            while (true) {
                try {
                   ...
                   // onLogReceived("<-> Requisi��o recebida.", LOG_LEVEL_INFO, null);
          
                    Host hostProxy = new Host(this.proxyAddr, this.proxyPort);
                    Host hostCliente = new Host(acceptCliente);
                    
                    InjectService reqInject = new InjectService(hostProxy, hostCliente, acceptCliente.getPort()) {
                        @Override
   ...
   public void onLogReceived(String log, int level, Exception e) {}
    ...
    public synchronized void onConnectionClosed(InjectService obj) {
